 ...
 * Riproduzione del grafico di fig.7 del paper "Dynamical Properties of a Boolean
 * Model of Gene Regulatory Network with Memory".
 *
 * Traccia:"Esperimento che lancia 4 volte Derrida. Si considerano 4 MDT:1,2,4,8; per 
 * ognuno di essi si creano 50 reti e 100 C.I per rete.
 * Struttura della rete: 
 * 100 G-P pairs, kin=2 (fisso) , random kout, bias=0.5."
 *
 * In questo esperimento viene chiamato l'esperimento di Derrida semplice, andando
 * a modificare ogni volta la memoria del sistema (MDT). Non ci sono limiti nè 
 * per numero reti generate, o condizioni, nè per MDT variabili.
...
const char INPUT_ESP[]="input_espfig7.txt";
...
const char OUTPUT_DERRIDA[]="output_esperimento.txt";

//comandi per le chiamate di system
const char invocaDerrida[]="/usr/bin/reti_gpbn/esp_derrida";
...
int main( int argc, char **argv){
	...
	if(!leggi_input(INPUT_ESP, &p)){
		printf("errore in lettura degli9 input dell'esp_fig2\n");
		return 1;}
	...
	for(i=0;i<p.n_MDT;i++){
		...
		if(!copia_file_loc(INPUT_GS, nome_serie_aus,1, p.nome_serie, p.l_MDT[i])){
			...
			return -1;}
		//lancia Derrida
		if(system(invocaDerrida)!=0){
			printf("Errore!! esecuzione fallita di %s\n",invocaDerrida);
			return -1;}
		//copia solo il risultato
		if(!copia_file_loc(OUTPUT_DERRIDA, nome_serie_aus,1, p.nome_serie, p.l_MDT[i])){
			printf("errore in copia_file_loc %s\n",OUTPUT_DERRIDA);
			return -1;}
		}
